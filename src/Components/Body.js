...
const Body = () => {
  ...
  return RestroData?.length === 0 ? (
    ...
    <div className="text-center mt-6 h-[80%] w-[100%]" >
      ...
      />
      <button data-testid="searchBtn" className="h-8 w-20 ml-4 bg-gray-600 text-white rounded-lg hover:bg-green-500  xsm:h-6,w-15 mob:h-6,w-15"
        onClick={() => {
          const data = filterData(SearchData, RestroData);
          setFilteredRestaurants(data);
        }}>Search</button>
      <div className="flex flex-wrap justify-center gap-8 border-none mb-6" data-testid="res-list">
