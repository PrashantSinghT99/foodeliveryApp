...
// console.log(menuData);
  ...
  <div className="flex h-80 w-screen bg-blue-900 justify-center text-center">
    <div>
      ...
      <div className="px-2 py-20 mt-4 text-white font-bold text-2xl xsm:text-md">
      <h1 >{name}</h1>
      <h2 className="text-xl xsm:text-md"><span>🔴</span>{areaName},{city}</h2>
      <h2 className="text-xl xsm:text-md"><span>⭐</span>{avgRatingString}</h2>
      <h2 className="text-xl xsm:text-md"><span>🍟</span>{costForTwoMessage}</h2>
    </div>
    </div>
  );
};
